...
class VendorDetector:
    ...
    def analyze_url_for_vendor(self, url: str, city_variations: List[str]) -> Optional[VendorMatch]:
    ...
    def calculate_confidence(self, slug: str, city_variations: List[str], base_confidence: float) -> float:
    ...
    def discover_vendor(self, city_name: str, state: str) -> Optional[VendorMatch]:
        ...
        print(f"Best match: {best_match.vendor} (confidence: {best_match.confidence:.2f})")
...
def main():
    ...
    for city_key, match in results.items():
        print(f"{city_key}: {match.vendor} ({match.city_slug}) - {match.confidence:.2f}")
