...
from adapters import PrimeGovAdapter, CivicClerkAdapter, LegistarAdapter, GranicusAdapter, NovusAgendaAdapter
...
class BackgroundProcessor:
        ...
        for vendor, vendor_cities in by_vendor.items():
            ...
            for city in sorted_cities:
                ...
                if not self._should_sync_city(city):
                    ...
                    city_banana = city.get('city_banana') or generate_city_banana(city.get('city_name'), city.get('state'))
                ...
                logger.info(f"Sync completed for {city.get('city_name', 'unknown')}: {result.status}")
            ...
            logger.info(f"Polite sync completed in {duration:.1f}s: {total_meetings} meetings found, {total_processed} processed, {failed_count} cities failed")
    ...
    def _sync_city(self, city_info: Dict[str, Any]) -> SyncResult:
        ...
        city_banana = city_info.get('city_banana') or generate_city_banana(city_name, state)
        ...
        try:
            ...
            # Scrape ALL meetings first (for user display) - simple and direct like gatest.py
            ...
            logger.info(f"Found {len(all_meetings)} total meetings for {city_name}, {len(meetings_with_packets)} have packets")
            ...
            for meeting in all_meetings:
                ...
                try:
                    ...
                    # NEW: Enqueue for processing instead of processing inline
                    if meeting.get("packet_url"):
                        # Calculate priority based on meeting date
                        days_old = 0
                        if meeting_data.get("meeting_date"):
                            try:
                                meeting_datetime = datetime.fromisoformat(meeting_data["meeting_date"])
                        ...
                        queue_id = self.db.enqueue_for_processing(
                            packet_url=meeting["packet_url"],
                            meeting_id=meeting_data["meeting_id"],
                            city_banana=meeting_data["city_banana"],
                            priority=priority,
                            metadata={"title": meeting.get("title", "Unknown")}
                        )
                        
                        if queue_id > 0:
                            logger.info(f"Enqueued meeting '{meeting.get('title')}' for processing (priority: {priority})")
                        else:
                            logger.debug(f"Meeting '{meeting.get('title')}' already in processing queue")
                    else:
            ...
            # Skip sync status updates (no locks needed)
    ...
    def _sync_city_with_retry(self, city_info: Dict[str, Any], max_retries: int = 2) -> SyncResult:
        """Sync city with retry (5s, 20s delays)"""
        city_name = city_info.get('city_name', 'unknown')
        city_banana = city_info.get('city_banana') or generate_city_banana(city_name, city_info.get('state'))
    ...
    def _prioritize_cities(self, cities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def get_priority(city):
            try:
                ...
                city_banana = city.get('city_banana') or generate_city_banana(city_name, state)
    ...
    def _process_unprocessed_meetings(self, limit=20):
        """Process meetings from the processing queue"""
        logger.info("Processing meetings from queue...")
        
        # First, ensure unprocessed meetings are in the queue
        enqueued = self.db.bulk_enqueue_unprocessed_meetings(limit=100)
        if enqueued > 0:
            logger.info(f"Added {enqueued} new meetings to processing queue")
        
        # Reset any failed items that can be retried
        reset_count = self.db.reset_failed_items(max_retries=3)
        if reset_count > 0:
        ...
        if pending_count == 0:
            logger.debug("No pending items in processing queue")
            return
        
        logger.info(f"Processing up to {limit} items from queue")
        
        # Process items from the queue
        if self.processor:
            # Collect items for batch processing
            queue_items = []
            queue_item_map = {}  # Map queue_id to item data
            
            for _ in range(min(limit, pending_count)):
                item = self.db.get_next_for_processing()
                if not item:
                    break
                    
                queue_items.append(item)
                queue_item_map[item['id']] = item
            
            if not queue_items:
                logger.warning("Could not retrieve any items from queue")
                return
            
            logger.info(f"Retrieved {len(queue_items)} items from queue for processing")
            
            # Process in batches for efficiency
            if len(queue_items) >= 5:  # Use batch API for 5+ items
                try:
                    # Prepare batch requests
                    batch_requests = []
                    custom_id_to_queue_id = {}  # Map custom_id to queue_id for result processing
                    
                    for item in queue_items:
                        custom_id = f"queue_{item['id']}_{item['city_banana']}"
                        batch_requests.append({
                            "url": item["packet_url"],
                            "custom_id": custom_id
                        })
                        custom_id_to_queue_id[custom_id] = item['id']
                    
                    logger.info(f"Submitting {len(batch_requests)} PDFs to batch API")
                    ...
                    if isinstance(batch_results, list):
                        for result in batch_results:
                            try:
                                ...
                                queue_id = custom_id_to_queue_id.get(custom_id)
                                queue_item = queue_item_map.get(queue_id)
                                
                                if not queue_item:
                                    logger.warning(f"No queue item found for custom_id: {custom_id}")
                                    continue
                                
                                if result.result_type.value == "succeeded" and result.message:
                                    # Extract summary
                                    ...
                                    if content and isinstance(content, list) and len(content) > 0:
                                        ...
                                        if summary:
                                            # Store summary
                                            meeting = self.db.get_meeting_by_packet_url(queue_item["packet_url"])
                                            if meeting:
                                                self.db.store_meeting_summary(
                                                    meeting,
                                                    summary,
                                                    0.0  # Processing time not tracked for batch API
                                                )
                                            self.db.mark_processing_complete(queue_id)
                                            success_count += 1
                                            logger.info(f"Processed and stored summary for queue item {queue_id}")
                                        else:
                                            self.db.mark_processing_failed(queue_id, "Empty summary received")
                                else:
                                    error_msg = getattr(result, 'error', 'Unknown error')
                                    self.db.mark_processing_failed(queue_id, f"Batch API error: {error_msg}")
                                    
                            except Exception as e:
                                logger.error(f"Error processing batch result: {e}")
                                if queue_id:
                                    self.db.mark_processing_failed(queue_id, str(e))
                    
                    logger.info(f"Batch processing completed: {success_count}/{len(batch_requests)} successful")
                    
                except Exception as e:
                    logger.error(f"Batch API submission failed: {e}")
                    # Mark all items as failed
                    for item in queue_items:
                        self.db.mark_processing_failed(item['id'], f"Batch submission error: {str(e)}")
            else:
                # Process individually for small batches
                logger.info(f"Processing {len(queue_items)} items individually")
                for item in queue_items:
                    try:
                        # Get meeting data for context
                        meeting_data = {
                            "packet_url": item['packet_url'],
                            "city_banana": item['city_banana'],
                            "meeting_id": item['meeting_id']
                        }
                        
                        # Process the meeting
                        self.processor.process_agenda_with_cache(meeting_data)
                        self.db.mark_processing_complete(item['id'])
                        logger.info(f"Successfully processed queue item {item['id']}")
                        
                    except Exception as e:
                        logger.error(f"Error processing queue item {item['id']}: {e}")
                        self.db.mark_processing_failed(item['id'], str(e))
        else:
            logger.warning("No processor available for PDF processing")
    ...
    def process_all_unprocessed_meetings(self, batch_size=20):
        ...
        while True:
            ...
            if not unprocessed:
                logger.info(f"No more unprocessed meetings found. Total processed: {total_processed}")
                break
            ...
            batch_processed = batch_size if not remaining else batch_size - len(remaining)
        ...
        logger.info(f"Finished processing all unprocessed meetings. Total: {total_processed}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--process-all-unprocessed", action="store_true", help="Process ALL unprocessed meetings")
