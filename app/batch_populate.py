...
class BatchPopulator:
    ...
    def get_existing_cities(self) -> set:
        ...
        try:
            ...
            return {
                f"{city['city_name'].lower()},{city['state'].lower()}"
    ...
    def process_single_city(
        self, city_data: Dict, discover_vendor: bool = True
    ) -> bool:
        ...
        try:
            ...
            if discover_vendor:
                try:
                    vendor_match = self.vendor_detector.discover_vendor(
                        city_name, state
                    )
            ...
            entry_data = {
                ...
                "all_zipcodes": [city_data.get("primary_zipcode")]
                if city_data.get("primary_zipcode")
                else [],
                "meetings": [],
            }
            ...
            print(
                f"✓ {city_key} (vendor: {vendor or 'unknown'}, slug: {entry_data['city_slug']})"
            )
            return True
    ...
    def populate_batch_sequential(
        self, cities: List[Dict], discover_vendors: bool = True
    ):
    ...
    def populate_batch_parallel(
        ...
        new_cities = [
            city
            for city in cities
            if f"{city['city_name'].lower()},{city['state'].lower()}"
            not in existing_cities
        ]
    ...
    def run_full_population(
        ...
        if priority_first:
            priority_keys = {
                f"{c['city_name'].lower()},{c['state'].lower()}"
        ...
        print(
            f"Success rate: {stats['successful_stores'] / max(stats['total_processed'], 1) * 100:.1f}%"
        )
...
def main():
    ...
    parser.add_argument(
        "--no-vendors", action="store_true", help="Skip vendor discovery (faster)"
    )
    parser.add_argument(
        "--no-priority", action="store_true", help="Skip priority cities first"
    )
    parser.add_argument(
        "--test", action="store_true", help="Test run with small subset of cities"
    )
