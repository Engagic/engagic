...
class CityPopulator:
    ...
    def search_city_council_website(
        self, city_name: str, state: str
    ...
    def analyze_url_for_vendor(
        ...
        try:
            ...
            if any(suffix in domain for suffix in [".gov", ".org", ".us"]):
                if city_name.lower().replace(" ", "") in domain.replace(
                    "-", ""
                ).replace("_", ""):
    ...
    def populate_single_city(self, city_data: Dict) -> bool:
        ...
        try:
            ...
            print(
                f"✓ Stored: {city_name}, {state} (vendor: {vendor or 'unknown'}, slug: {final_slug})"
            )
            return True
        except Exception as e:
    ...
    def update_missing_vendors(self):
        ...
        for i, city in enumerate(missing_vendor_cities, 1):
            print(
                f"[{i}/{len(missing_vendor_cities)}] Updating {city['city_name']}, {city['state']}"
            )

            vendor, slug = self.search_city_council_website(
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of cities to process (for testing)"
    )
    parser.add_argument(
        "--update-vendors", action="store_true", help="Update missing vendor info only"
    )
