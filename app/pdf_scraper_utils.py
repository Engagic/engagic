...
def deep_scrape_pdfs(detail_url: str, base_url: str = None, max_depth: int = 2) -> List[str]:
    """
    ...
    def _scrape_page(url: str, depth: int = 0, parent_url: str = None):
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if (full_url_normalized == original_url_normalized or 
                    full_url_path == original_path or
                    (parent_url and full_url_normalized == parent_url.rstrip('/').lower())):
...
def is_pdf_url(url: str, link_text: str = "") -> bool:
    ...
    if link_text:
        ...
        has_pdf_indicator = any(indicator in text_lower for indicator in pdf_text_indicators)
