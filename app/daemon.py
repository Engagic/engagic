...
class EngagicDaemon:
    ...
    def show_status(self):
        ...
        try:
            ...
            old_queue_stats = self.processor.db.get_processing_queue_stats()
            print("\nLegacy Processing Stats:")
            print(f"  Success rate: {old_queue_stats['success_rate']:.1f}%")
            print(f"  Recent meetings (24h): {old_queue_stats['recent_count']}")
            print(f"  Total with packet URLs: {old_queue_stats['total_meetings']}")
            
            # Show new processing queue stats
            new_queue_stats = self.processor.db.get_queue_stats()
            print("\nProcessing Queue:")
            print(f"  Pending: {new_queue_stats.get('pending_count', 0)}")
            print(f"  Processing: {new_queue_stats.get('processing_count', 0)}")
            print(f"  Completed: {new_queue_stats.get('completed_count', 0)}")
            print(f"  Failed: {new_queue_stats.get('failed_count', 0)}")
            print(f"  Permanently failed: {new_queue_stats.get('permanently_failed', 0)}")
            if new_queue_stats.get('avg_processing_seconds', 0) > 0:
                print(f"  Average processing time: {new_queue_stats['avg_processing_seconds']:.1f}s")
