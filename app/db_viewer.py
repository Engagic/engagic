...
class DatabaseViewer:
    ...
    def show_zipcode_table(self):
        ...
        with self.db.get_connection() as conn:
            ...
            # Headers
            headers = ["ID", "Zipcode", "City Name", "City Slug", "State", "County", "Created", "Last Accessed"]
            print("\n=== ZIPCODE ENTRIES ===")
    ...
    def show_meetings_table(self, limit=20):
        ...
        with self.db.get_connection() as conn:
            ...
            # Headers
            headers = ["ID", "Vendor", "City Name", "City Slug", "Date", "Meeting Name", "Packet URL", "Created"]
            print(f"\n=== MEETINGS (last {limit}) ===")
            print(" | ".join(f"{h:<15}" for h in headers))
            print("-" * (len(headers) * 17))
    ...
    def update_meeting_entry(self, entry_id: int, field: str, new_value: str):
        ...
        valid_fields = ["vendor", "city_name", "city_slug", "meeting_date", "meeting_name", "packet_url"]
    ...
    def add_zipcode_entry(self, zipcode: str, city_name: str, city_slug: str, state: str = "", county: str = ""):
    ...
    def delete_zipcode_entry(self, entry_id: int):
        ...
        with self.db.get_connection() as conn:
            try:
                cursor = conn.execute("DELETE FROM zipcode_entries WHERE id = ?", (entry_id,))
                if cursor.rowcount == 0:
                    print(f"No entry found with ID {entry_id}")
                    return False
                else:
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not entry_id.isdigit():
                print("Invalid ID")
                continue
            field = input("Enter field to update (vendor/city_name/city_slug/meeting_date/meeting_name/packet_url): ").strip()
            new_value = input(f"Enter new value for {field}: ").strip()
