...
class DatabaseManager:
    ...
    def __init__(
        self, locations_db_path: str, meetings_db_path: str, analytics_db_path: str
    ):
    ...
    def add_city(
        ...
        return self.locations.add_city(
    ...
    def delete_city(self, city_banana: str) -> bool:
        ...
        if locations_success:
            logger.info(
                f"Deleted city {city_banana} and {meetings_deleted} associated meetings"
            )
    ...
    def store_meeting_summary(
        ...
        return self.meetings.store_meeting_summary(
    ...
    def clear_meeting_summaries(
        ...
        return self.meetings.clear_meeting_summaries(
            city_banana, meeting_ids, all_meetings
        )
    ...
    def log_search(
        ...
        if city_id:
            with self.locations.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT city_banana FROM cities WHERE id = ?", (city_id,)
                )
                result = cursor.fetchone()
                if result:
                    city_banana = result["city_banana"]

        return self.analytics.log_search(
            search_query, search_type, city_banana, zipcode, topic_flags
        )
    ...
    def log_city_request(
        ...
        return self.analytics.log_city_request(
            city_name, state, search_query, search_type, zipcode, user_ip
        )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            locations_health = {
                "status": "healthy",
                "tables": len(self.locations.get_db_stats()["tables"]),
            }
        ...
        try:
            meetings_health = {
                "status": "healthy",
                "tables": len(self.meetings.get_db_stats()["tables"]),
            }
        except Exception as e:
        ...
        try:
            analytics_health = {
                "status": "healthy",
                "tables": len(self.analytics.get_db_stats()["tables"]),
            }
        except Exception as e:
        ...
        if any(
            db["status"] == "error"
            for db in [locations_health, meetings_health, analytics_health]
        ):
