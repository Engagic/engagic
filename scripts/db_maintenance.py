...
class DatabaseMaintenance:
    ...
    def vacuum_databases(self):
        ...
        for name, path in databases:
            try:
                ...
                print(f"{name:12} - Before: {size_before:.2f}MB, After: {size_after:.2f}MB, Saved: {saved:.2f}MB")
    ...
    def clean_old_analytics(self, days=30):
        ...
        with self.db.analytics.get_connection() as conn:
            ...
            print(f"Usage metrics: {metrics_before} → {metrics_before - metrics_deleted} (deleted {metrics_deleted})")
    ...
    def fix_city_banana_values(self):
        ...
        with self.db.locations.get_connection() as conn:
            ...
            for city in cities:
                ...
                expected_banana = re.sub(r'[^a-zA-Z0-9]', '', city['city_name']).lower() + city['state'].upper()
                
                if city['city_banana'] != expected_banana:
                    updates_needed.append((city['id'], city['city_banana'], expected_banana))
    ...
    def clean_orphaned_meetings(self):
        ...
        with self.db.meetings.get_connection() as conn:
            ...
            for banana in list(orphaned)[:10]:
                cursor.execute(
                    "SELECT COUNT(*) FROM meetings WHERE city_banana = ?",
                    (banana,)
                )
