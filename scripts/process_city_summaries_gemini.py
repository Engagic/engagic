#!/usr/bin/env python3
"""
Process all cached packet URLs for a specific city using Gemini
Usage: python scripts/process_city_summaries_gemini.py <city_banana>
"""
...
def process_city_meetings(city_banana: str):
    """Process all meetings with packet URLs for a specific city using Gemini"""
    ...
    try:
        # Try to use GEMINI_API_KEY first, then fall back to general LLM_API_KEY
        api_key = config.GEMINI_API_KEY or config.get_api_key()
        processor = GeminiProcessor(api_key=api_key)
        logger.info(f"Initialized Gemini processor for city: {city_banana}")
    ...
    logger.info("\nUsing Gemini models for processing")


def main():
    if len(sys.argv) != 2:
        print("Usage: python scripts/process_city_summaries_gemini.py <city_banana>")
        print("Example: python scripts/process_city_summaries_gemini.py paloaltoCA")
        print("\nMake sure to set GEMINI_API_KEY environment variable")
        sys.exit(1)
    
    city_banana = sys.argv[1]
    
    logger.info(f"Starting Gemini processing for city: {city_banana}")
