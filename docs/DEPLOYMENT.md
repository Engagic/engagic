...
```nginx
# Include rate limit blocked IPs (at http level, before server blocks)
...
server {
    ...
    # Deny blocked IPs from rate limiter
    if ($blocked_ip) {
...
**Note:** The `blocked_ips_nginx.conf` file is auto-generated by the API when it bans IPs. Format is a complete nginx geo block that sets `$blocked_ip=1` for banned IPs.
