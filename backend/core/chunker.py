...
class AgendaChunker:
    ...
    def chunk_by_patterns(self, text: str, max_chunk_size: int = 75000) -> List[Dict[str, Any]]:
        """
        ...
        if found_start_marker and found_end_marker:
            ...
            logger.debug(f"[Chunker] Using marker-based range: {agenda_start}-{agenda_end}")
        else:
            actual_agenda = agenda_section
            agenda_start = 0
            agenda_end = agenda_section_size
            logger.debug("[Chunker] No clear markers found, searching full agenda section")
        ...
        if not agenda_items:
            logger.info(f"[Chunker] No agenda items found in agenda section (searched {len(actual_agenda)} chars between markers)")
            return []

        logger.info(f"[Chunker] Found {len(agenda_items)} items in agenda section (between positions {agenda_start}-{agenda_end})")
        ...
        for item in agenda_items:
            ...
            if match:
                ...
                logger.debug(f"[Chunker] Found '{item['number']}. {item['title'][:40]}...' at position {boundary_pos}")
            else:
                ...
                if match:
                    boundary_pos = search_start + match.start()
                    split_points.append(boundary_pos)
                    logger.debug(f"[Chunker] Found item {item['number']} (by number only) at position {boundary_pos}")

        split_points = sorted(set(split_points))
        split_points.append(len(text))

        logger.info(f"[Chunker] Found {len(split_points) - 2} boundaries in {len(text)} chars document")

        # If too few boundaries found, not worth chunking
        if len(split_points) < 3:  # Need at least 2 items (0, item1, end)
            logger.info(f"[Chunker] Only {len(split_points) - 2} boundaries found - processing monolithically")
            return []
        ...
        for chunk in chunks:
            ...
            if len(chunk['text']) < 1000 and chunk['start_pos'] == 0:
                logger.debug(f"[Chunker] Skipping small header chunk ({len(chunk['text'])} chars)")
                continue
            meaningful_chunks.append(chunk)

        if len(meaningful_chunks) <= 1:
            logger.info(f"[Chunker] Only {len(meaningful_chunks)} meaningful chunks - processing monolithically")
            return []

        # Cap at reasonable number of chunks
        if len(meaningful_chunks) > 50:
            logger.warning(f"[Chunker] {len(meaningful_chunks)} chunks detected - too many! Processing monolithically")
            return []

        logger.info(f"[Chunker] Created {len(meaningful_chunks)} chunks from {len(agenda_items)} agenda items")
    ...
    def _detect_cover_end(self, text: str) -> int:
        ...
        if found_pattern:
            logger.debug(f"[Chunker] Found pattern '{found_pattern}' at position {earliest_pos}")
