...
export const load: PageServerLoad = async ({ locals, setHeaders }) => {
	...
	try {
		...
		// Filter to cities with council members only
		const cities = data.cities.filter(c => c.council_member_count > 0);
		...
		throw error(500, 'Failed to load council member data');
	}
};
